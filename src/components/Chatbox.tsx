...
export function Chatbox({ messages, onSendMessage, isDrawer, title, showInput = true }: ChatboxProps) {
    ...
    return (
        <div className="flex flex-col h-full bg-pd-surface rounded-2xl overflow-hidden shadow-sm">
            ...
            <div ref={scrollRef} className="flex-1 overflow-y-auto p-2 md:p-4 space-y-2 md:space-y-3">
                {messages.map((msg) => (
                    <div key={msg.id} className="text-xs md:text-sm">
                        {msg.isSystem ? (
                            <div className="bg-pd-green/15 text-pd-green rounded-lg p-2 text-center font-bold animate-in fade-in slide-in-from-bottom-1">
                                {msg.playerName} {msg.message || `guessed the word! (+${msg.scoreGained})`}
                            </div>
                        ) : (
                            <div className="flex gap-1.5 md:gap-2 break-words items-baseline">
